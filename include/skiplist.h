...
public:
	SkipList(const SortField& sort_field, const Value& value)
		: max_level_(0)
		, node_count_(0) {
		header_ = new Node<SortField, Value>(MAX_LEVEL, sort_field, value);
		footer_ = new Node<SortField, Value>(0, sort_field, value);
...
template <typename SortField, typename Value>
const Node<SortField, Value>* SkipList<SortField, Value>::find(
	const SortField& sort_field, int* rank) const {
	if (rank != nullptr)
		*rank = 1;
	auto node = header_;
